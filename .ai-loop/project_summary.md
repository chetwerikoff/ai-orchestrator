...
C08: task-first preflight on `## Files in scope` (`Test-TaskFilesInScopeExist`, `-SkipScopeCheck`), validation tests, project summary refresh.

## Next Likely Steps

1. **Reinstall target projects after C08:** from this repo run `.\scripts\install_into_project.ps1 -TargetProject <path>` so they pick up `ai_loop_task_first.ps1`; verify with `rg -n "SkipScopeCheck|Test-TaskFilesInScopeExist" <target>\scripts\ai_loop_task_first.ps1`.
2. H2N target project cleanup: D01 (compact project_summary.md) → D02 (reinstall scripts + AGENTS.md) → D03 (compact REVIEW_STATE.md, manual).
3. Use task-first for new work; use `continue_ai_loop.ps1` for interrupted loops; optionally pass `-WithWrapUp`, then periodically run `promote_session.ps1` to refresh `failures.md`.
