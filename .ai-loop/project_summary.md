...
- `templates/` -- target-project scaffolds, including `implementer_summary_template.md`, `codex_review_prompt.md`, `planner_prompt.md`, `reviewer_prompt.md` (narrow advisory planner reviewer text; unrelated to implementation `codex_review_prompt.md`), `user_ask_template.md` (hint template copied to `.ai-loop/user_ask_template.md` by the installer, not to `user_ask.md`, which users create or copy manually), `project_summary.md`, `task.md`, and `opencode.json`.
- `scripts/install_into_project.ps1` -- copies orchestrator scripts (including `ai_loop_plan.ps1`, `run_claude_planner.ps1`, `run_codex_reviewer.ps1`, `run_scout_pass.ps1` and `run_opencode_scout.ps1` for DD-022 `-WithScout`, plus `wrap_up_session.ps1` and `promote_session.ps1` for DD-023) and planner/templates (`planner_prompt.md`, `reviewer_prompt.md`, `user_ask_template.md`) into a target project; refuses self-install when source and target are the same directory; when adding or removing files under `templates/`, update this script so installs stay complete (see AGENTS.md templates contract).
