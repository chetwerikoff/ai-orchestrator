...
**`ai_loop_auto.ps1` staging model (DD-024):** **DurableAlwaysCommit** is the `.ai-loop/` subset of that list (task/summary/repo_map/failures, archive rolls dir, optional session draft) and is always eligible to stage. **ActiveScope** is the path list parsed from `## Files in scope` in `.ai-loop/task.md` (missing section or no bullets → empty). **`Stage-SafeProjectFiles`** stages DurableAlwaysCommit when the path exists, then each ActiveScope entry that is also allowed by `SafeAddPaths` (and not already covered by durable paths); it never stages whole directory blobs like `tasks/` or `scripts/` unless they appear verbatim in ActiveScope or as durable paths. **Fail-closed:** empty ActiveScope stages only durable paths and emits a `[scope-filter]` warning so parallel out-of-scope files (for example extra `tasks/*.md`) are not pulled into the commit. **Codex** still receives unfiltered `git diff`/`diff --stat`; only `git_status.txt` is narrowed to the same stage set. Parallel untracked files such as `tasks/user_ask_*.md` created by other agents are excluded from both `git_status.txt` and the staged commit set whenever the active task's `## Files in scope` omits `tasks/`.
